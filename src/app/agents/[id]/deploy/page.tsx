...
export default function AgentDeployPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function loadDeployments() {
    try {
      ...
      const list = (d.deployments || []).filter((dep: any) => dep.agent_id === agentId);
      if (list.length > 0) {
        setDeployments(list);
      } else {
        setDeployments([
          {
            id: `dep_${agentId}_web_01`,
            agent_id: agentId,
            name: 'Production Website Widget',
            channel: 'WEBSITE',
            environment: 'PRODUCTION',
            public_key: `pk_live_${agentId}_widget`,
            status: 'ACTIVE',
          ...
          {
            id: `dep_${agentId}_rest_02`,
            agent_id: agentId,
            name: 'Customer Support REST API',
            channel: 'REST_API',
            environment: 'PRODUCTION',
            public_key: `pk_live_${agentId}_rest`,
            status: 'ACTIVE',
      ...
      console.error('Failed to load agent deployments:', err);
    } finally {
      setLoading(false);
    }
  }

  async function toggleStatus(dep: any) {
  ...
  async function handleCreateDeployment(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/deployments', {
        ...
        body: JSON.stringify({
          agent_id: agentId,
          name: newDepName || `${newChannel === 'WEBSITE' ? 'Website Widget' : newChannel === 'REST_API' ? 'REST API Endpoint' : 'Mobile SDK'}`,
          channel: newChannel,
  ...
  const activeIframeCount = deployments.filter(d => d.channel === 'IFRAME' && d.status === 'ACTIVE').length;
            ...
            {savedSuccess && (
              <div className="bg-emerald-950/70 border border-emerald-800/80 text-emerald-300 px-4 py-2.5 rounded-xl text-xs flex items-center justify-between shadow-lg animate-in fade-in slide-in-from-top-2">
                <div className="flex items-center gap-2">
                  ...
                  <span>Agent deployment channels and settings saved successfully!</span>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-zinc-400 mt-0.5">
                      Choose how users interact with agent &quot;{agentId}&quot; across web, mobile, and API endpoints.
                    </p>
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
                            ...
                            >
                              {copiedSnippet === dep.id ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />}
                            </button>
                          </div>
                        </div>
                      );
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-1.5">
                          {['#8b5cf6', '#3b82f6', '#10b981', '#f43f5e', '#f59e0b'].map((hex) => (
                            <button
                              key={hex}
                              type="button"
                  ...
                  <div className="p-6 rounded-2xl bg-[#121318] border border-zinc-800 space-y-4">
                    ...
                    <div className="h-96 rounded-xl bg-zinc-950 border border-zinc-800/80 p-4 relative overflow-hidden flex flex-col justify-end">
                      ...
                      <div className={`absolute bottom-3 ${position === 'bottom-right' ? 'right-3' : 'left-3'}`}>
                        <button
                          className="px-3.5 py-2 rounded-full text-white text-xs font-semibold shadow-xl flex items-center gap-2"
                ...
                <div className="p-6 rounded-2xl bg-[#121318] border border-zinc-800 space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-bold text-white flex items-center gap-2">
                      ...
                      <span>Website Floating Script Tag</span>
                    </h3>
                    <button
                      onClick={() => {
                        const snippet = `<script src="${origin || 'http://localhost:3000'}/agent.js" data-agent-id="${agentId}" defer></script>`;
...
{`<script src="${origin || 'http://localhost:3000'}/agent.js" data-agent-id="${agentId}" defer></script>`}
                  </pre>
                </div>
                ...
                <div className="p-6 rounded-2xl bg-[#121318] border border-zinc-800 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => {
                        const snippet = `curl -X POST ${origin || 'http://localhost:3000'}/api/v1/agents/${agentId}/chat \\\n  -H "Authorization: Bearer pk_live_${agentId}_rest" \\\n  -H "Content-Type: application/json" \\\n  -d '{"message": "Check order #10842"}'`;
...
{`curl -X POST ${origin || 'http://localhost:3000'}/api/v1/agents/${agentId}/chat \\
  -H "Authorization: Bearer pk_live_${agentId}_rest" \\
  -H "Content-Type: application/json" \\
  -d '{"message": "Check order #10842"}'`}
                  </pre>
                ...
                <div className="bg-[#121318] border border-zinc-800 rounded-2xl w-full max-w-lg p-6 space-y-5 shadow-2xl animate-in fade-in zoom-in-95">
                  <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
                    <h3 className="text-sm font-bold text-white flex items-center gap-2">
                      ...
                      <span>Deploy Channel for {agentId}</span>
                    </h3>
                  ...
                  <form onSubmit={handleCreateDeployment} className="space-y-4">
                    <div className="space-y-1.5">
                      ...
                      <input
                        type="text"
                        placeholder="e.g. Website Live Widget"
                        value={newDepName}
